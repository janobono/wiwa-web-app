...
import { WiwaButton, WiwaFileInput, WiwaSpinner } from '../../../ui';
...
const ChangeLogoDialog: React.FC<ChangeLogoDialogProps> = (props) => {
    ...
    useEffect(() => {
        setLogoMessage(t(RESOURCE.COMPONENT.PAGE.CONFIG.DIALOG.CHANGE_LOGO.FORM.LOGO.VALIDATION_MESSAGE1).toString());
    }, []);
    ...
    const handleSubmit = async () => {
        ...
        try {
            if (isFormValid() && logo) {
                ...
                if (wiwaError) {
                    setError(t(RESOURCE.COMPONENT.PAGE.CONFIG.DIALOG.CHANGE_LOGO.ERROR).toString());
                } else {
                        ...
                        >
                            <Dialog.Panel
                                ...
                                <div className="text-lg md:text-xl font-bold text-center mb-5">
                                    {t(RESOURCE.COMPONENT.PAGE.CONFIG.DIALOG.CHANGE_LOGO.TITLE)}
                                </div>
                                <form
                                    onSubmit={(event => {
                                    ...
                                    <div className="mb-5">
                                        <WiwaFileInput
                                            label={t(RESOURCE.COMPONENT.PAGE.CONFIG.DIALOG.CHANGE_LOGO.FORM.LOGO.LABEL)}
                                            message={logoMessage}
                                            required={true}
                                            setValue={(file) => {
                                                if (!file) {
                                                ...
                                                if (!(file.type === 'image/jpg' || file.type === 'image/png')) {
                                                    setLogo(undefined);
                                                    setLogoValid(false);
                                                    setLogoMessage(t(RESOURCE.COMPONENT.PAGE.CONFIG.DIALOG.CHANGE_LOGO.FORM.LOGO.VALIDATION_MESSAGE2).toString());
                                                    return;
                                                }
                                                setLogo(file);
                                                setLogoValid(true);
