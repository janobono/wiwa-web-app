            ...
            <div className="container p-5 mx-auto">
                <div className="flex flex-col items-center justify-center">
                    <div className="text-lg md:text-xl font-bold text-center mb-5">
                        {t(RESOURCE.COMPONENT.PAGE.AUTH.USER_ACCOUNT.TITLE)}
                    </div>
                    <div
                        className="grid grid-cols-1 md:grid-cols-3 place-items-start gap-2 w-full text-xs md:text-base items-center">

                        <div className="font-bold">{t(RESOURCE.COMPONENT.PAGE.AUTH.USER_ACCOUNT.USERNAME)}</div>
                        <div>{authState?.user?.sub}</div>
                        <div></div>

                        <div className="font-bold">{t(RESOURCE.COMPONENT.PAGE.AUTH.USER_ACCOUNT.PASSWORD)}</div>
                        <div>******</div>
                        <WiwaButton
                        ...
                        <div className="font-bold">{t(RESOURCE.COMPONENT.PAGE.AUTH.USER_ACCOUNT.EMAIL)}</div>
                        ...
                        <div className="font-bold">{t(RESOURCE.COMPONENT.PAGE.AUTH.USER_ACCOUNT.TITLE_BEFORE)}</div>
                        <div>{authState?.user?.titleBefore}</div>
                        <div></div>

                        <div className="font-bold">{t(RESOURCE.COMPONENT.PAGE.AUTH.USER_ACCOUNT.FIRST_NAME)}</div>
                        <div>{authState?.user?.firstName}</div>
                        <div></div>

                        <div className="font-bold">{t(RESOURCE.COMPONENT.PAGE.AUTH.USER_ACCOUNT.MID_NAME)}</div>
                        <div>{authState?.user?.midName}</div>
                        <div></div>

                        <div className="font-bold">{t(RESOURCE.COMPONENT.PAGE.AUTH.USER_ACCOUNT.LAST_NAME)}</div>
                        <div>{authState?.user?.lastName}</div>
                        <div></div>

                        <div className="font-bold">{t(RESOURCE.COMPONENT.PAGE.AUTH.USER_ACCOUNT.TITLE_AFTER)}</div>
                        <div>{authState?.user?.titleAfter}</div>
                        <div></div>

                        <div className="font-bold">{t(RESOURCE.COMPONENT.PAGE.AUTH.USER_ACCOUNT.GDPR)}</div>
                        ...
                        <div className="bg-yellow-400 p-2 my-5">
                            <div
                                ...
                                <div className="text-xs md:text-base pr-1">
                                    {t(RESOURCE.COMPONENT.PAGE.AUTH.USER_ACCOUNT.CONFIRMATION_WARNING)}
                                </div>
                                <WiwaButton
                                    title={t(RESOURCE.COMPONENT.PAGE.AUTH.USER_ACCOUNT.SEND_CONFIRMATION).toString()}
