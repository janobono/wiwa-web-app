...
import { WiwaButton, WiwaCaptcha, WiwaFormInput, WiwaNewPassword, WiwaSpinner } from '../../../ui';
                ...
                if (wiwaError) {
                    switch (wiwaError.code) {
                        ...
                        case WiwaErrorCode.INVALID_CREDENTIALS:
                            ...
                            break;
                        default:
                            setError(t(RESOURCE.COMPONENT.PAGE.AUTH.DIALOG.CHANGE_PASSWORD.ERROR).toString());
                            break;
                    }
                } else {
                    setSubmitted(true);
                        ...
                        >
                            <Dialog.Panel
                                ...
                                <div className="text-lg md:text-xl font-bold text-center mb-5">
                                    {t(RESOURCE.COMPONENT.PAGE.AUTH.DIALOG.CHANGE_PASSWORD.TITLE)}
                                </div>
                                {submitted ?
                                    <>
                                        <div className="mb-5">
                                            {t(RESOURCE.COMPONENT.PAGE.AUTH.DIALOG.CHANGE_PASSWORD.MESSAGE).toString()}
                                        </div>
                                        <WiwaButton
                                            className="w-full"
                                        ...
                                        <form
                                            ...
                                            <div className="mb-2">
                                                <WiwaFormInput
                                                    type="password"
                                                    name="oldPassword"
                                                    label={t(RESOURCE.COMPONENT.PAGE.AUTH.DIALOG.CHANGE_PASSWORD.FORM.ACTUAL_PASSWORD.LABEL)}
                                                    placeholder={t(RESOURCE.COMPONENT.PAGE.AUTH.DIALOG.CHANGE_PASSWORD.FORM.ACTUAL_PASSWORD.PLACEHOLDER).toString()}
                                                    required={true}
                                                    value={oldPassword}
                                                    ...
                                                    validate={() => {
                                                        if (oldPassword.trim().length === 0) {
                                                            return {
                                                                valid: false,
                                                                message: t(RESOURCE.COMPONENT.PAGE.AUTH.DIALOG.CHANGE_PASSWORD.FORM.ACTUAL_PASSWORD.VALIDATION_MESSAGE).toString()
                                                            };
                                                        }
                                                        return {valid: true};
                                                    }}
