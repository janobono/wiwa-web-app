...
import { WiwaButton, WiwaCaptcha, WiwaSpinner } from '../../../ui';
    ...
    const handleSubmit = async () => {
        ...
        try {
            if (isFormValid() && captchaToken) {
                ...
                if (wiwaError) {
                    setError(t(RESOURCE.COMPONENT.PAGE.AUTH.DIALOG.RESEND_CONFIRMATION.ERROR).toString());
                } else {
                    setSubmitted(true);
                }
            }
                        ...
                        >
                            <Dialog.Panel
                                ...
                                <div className="text-lg md:text-xl font-bold text-center mb-5">
                                    {t(RESOURCE.COMPONENT.PAGE.AUTH.DIALOG.RESEND_CONFIRMATION.TITLE)}
                                </div>
                                {submitted ?
                                    <>
                                        <div className="mb-5">
                                            {t(RESOURCE.COMPONENT.PAGE.AUTH.DIALOG.RESEND_CONFIRMATION.MESSAGE)}
                                        </div>
                                        <WiwaButton
                                            className="w-full"
