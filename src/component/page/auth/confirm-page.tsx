    ...
    const confirm = async () => {
        ...
        try {
            if (token) {
                ...
                if (wiwaError) {
                    setError(t(RESOURCE.COMPONENT.PAGE.AUTH.CONFIRM.ERROR_MESSAGE).toString());
                }
            }
        } finally {
            setSubmitting(false);
                        ...
                        <>
                            {
                                error ? <div className="font-normal m-5 text-red-500">{error}</div>
                                    :
                                    <div
                                        className="font-normal m-5">{t(RESOURCE.COMPONENT.PAGE.AUTH.CONFIRM.SUBMITTED_MESSAGE)}
                                    </div>
                            }
